...
# Unigram
### Tokenization: 확률값이 높은 tokenization 조합을 찾음. (확률값이 같은 경우, 일관된 룰로 선택)
- pug 는 p+ug 혹은 pu+g 로 tokenize
   - p+u+g : 0.000389
   - p+ug : 0.0022676
   - pu+g : 0.0022676 

### vocab 생성: 모든 substring 들을 vocab 으로 등록하고 제거해나감. (제거했을때, loss 의 증가폭이 가장 적은 것을 지움)
...
- Init
   - Vocabulary: Vocabulary: [b, h, p, ##g, ##n, ##s, ##u, ##g, ##s]
